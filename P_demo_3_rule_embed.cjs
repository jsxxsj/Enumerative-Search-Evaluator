...
const rule_app = require("./results/rule4_func_app.json");
Eval_build.new_micro_rule(rule_app);
Eval_build.embed_micro_rule("app");

const rule_cond = require("./results/rule3_condexpr.json");
Eval_build.new_micro_rule(rule_cond);
Eval_build.embed_micro_rule("cond_expr");

const rule_binopGT = require("./results/rule5_binopGT.json")
Eval_build.new_micro_rule(rule_binopGT);
Eval_build.embed_micro_rule("binop>");

is = [];
i_A = [Eval_build.json_parse("{let f = (x =>(x>0? x*f(x-1) : 1)); f(4);}")];
// i_A = [Eval_build.json_parse("{let f = x => x>5; ((x,y)=>x(y))(f,20);}")];
i_E = [{},null]
console.log("################\n",
Eval_build.run_seq(i_A,i_S,i_E));
